    ...
    def next(self):
        ...
        for data in self.datas:  # Пробегаемся по всем запрошенным тикерам
            ...
            if self.p.symbols == '' or data._dataname in self.p.symbols:  # Если торгуем все тикеры или данный тикер
                ...
                # _kk = np.array(_dd)
                # _sma1 = bt.indicators.SMA(_kk, period=50)
                # _sma2 = bt.indicators.SMA(_kk, period=100)
                # print("[", data._dataname, _sma1[0], _sma2[0], "]")

                #_sma1 = bt.indicators.SMA(data.close, period=50); _sma2 = bt.indicators.SMA(data.close, period=100)
                # https://www.lfd.uci.edu/~gohlke/pythonlibs/#ta-lib
                # pip install TA_Lib-0.4.24-cp39-cp39-win_amd64.whl
                # _sma1 = bt.talib.SMA(data, timeperiod=50)
                # _sma2 = bt.talib.SMA(data, timeperiod=100)
                # print("[", data._dataname, _sma1[0], _sma2[0], "]")
                ...
                # _sma11 = bt.indicators.SimpleMovingAverage(data.close, period=50)
                # print(_sma11)
